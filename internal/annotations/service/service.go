...
func (s *annotationsService) GetFeatureStats(ctx context.Context, featureID int32) ([]model.FeatureStats, error) {

	if err := verifyFeatureExists(featureID); err != nil {
		return nil, err
	}
	return s.repo.GetFeatureStats(ctx, featureID)
}