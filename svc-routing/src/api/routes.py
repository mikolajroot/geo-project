...
from src.services.sa import simulated_annealing
from src.services.lkh import generate_lkh
...
def optimize_route(request: OptimizationRequest, incoming_request: Request):
    ...
    if request.algorithm not in ["A*", "nearest_neighbor", "genetic", "ant_colony","simulated_annealing", "lkh"]:
    ...
    match request.algorithm:
        ...
        case "simulated_annealing":
            ...
            ordered_indices, computation_time = simulated_annealing(start_idx, points, dist_matrix)

            if not ordered_indices:
                raise HTTPException(status_code=500, detail="Simulated Annealing failed")
        ...
        case "lkh":
            ...
            ordered_indices, computation_time = generate_lkh(start_idx, points, dist_matrix)

            if not ordered_indices:
                raise HTTPException(status_code=500, detail="LKH failed")
